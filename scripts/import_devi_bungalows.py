...
def import_devi_migration(json_file_path):
    ...
    print(f"Total Devi Bungalows Records loaded: {total_leads}")

    registry = Registry(db_name)
    with registry.cursor() as cr:
        env = api.Environment(cr, SUPERUSER_ID, {})

        # 1. Company Setup (Devi Bungalows)
        print("\n--- 1. Setting up Devi Bungalows Company ---")
        ...
        # 2. Salespersons Setup (Hemant Prajapati)
        print("\n--- 2. Setting up Salesperson (Hemant Prajapati) ---")
        ...
        # Clean existing leads for Devi Bungalows if any
        ...
        # 6. Import Leads
        print("\n--- 5. Importing Devi Bungalows Leads with Chatter & Activities ---")
        count = 0
        success_count = 0
        ...
        cr.commit()
        print(f"\n=======================================================")
        print(f"🎉 DEVI BUNGALOWS IMPORT COMPLETE: {success_count}/{total_leads} Records Successfully Imported!")
        print(f"=======================================================")


if __name__ == '__main__':
    json_path = sys.argv[1] if len(sys.argv) > 1 else '/www/wwwroot/diyacrm/devi_clean_odoo_migration.json'
    import_devi_migration(json_path)
